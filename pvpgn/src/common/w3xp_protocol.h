...
typedef struct{
  t_bnet_header        h;
  bn_int               count;
  bn_byte			   check_clanshort; // 0x00 = not used, 0x01 = clanshort already taken
...
#define CLIENT_W3XP_CLAN_DELREQ 0x73ff
typedef struct{
  t_bnet_header        h;
  bn_int               count;
} t_client_w3xp_clan_delreq PACKED_ATTR();

#define SERVER_W3XP_CLAN_DELREPLY 0x73ff
typedef struct{
  t_bnet_header        h;
  bn_int               count;
  bn_byte              result;   /* 0-- Success  1-- Exception raised  2-- Clan exists less than 1 week, cannot remove */
} t_server_w3xp_clan_delreply PACKED_ATTR();

#define SERVER_W3XP_CLAN_DELREPLY_RESULT_OK 0x0
#define SERVER_W3XP_CLAN_DELREPLY_RESULT_EXCEPTION 0x1
#define SERVER_W3XP_CLAN_DELREPLY_RESULT_FAILED 0x2
