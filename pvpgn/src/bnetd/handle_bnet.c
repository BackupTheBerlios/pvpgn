...
extern int handle_bnet_packet(t_connection * c, t_packet const * const packet)
{
	...
	case CLIENT_CREATEACCOUNT_W3:
		...
		break; /* end CLIENT_CREATEACCOUNT_W3 */
 ...
 /* TODO: if type is TOSFILE make bnetd to send default tosfile if selected is not found */ 
	...
	switch (packet_get_type(packet))
	// ADDED 5/14/02 THEUNDYING - FRIENDS LIST
	// [zap-zero] 20020516
	{
	...
	// [quetzal] 20020823 - rewritten this "function" :)
	...
	case CLIENT_ARRANGEDTEAM_ACCEPT_DECLINE_INVITE: //if user invited Declines invite it sends this packet
		{
			char const *inviter;
			t_connection * dest_c;
	...
	case CLIENT_FINDANONGAME:
		...
		else if (bn_byte_get(packet->u.client_findanongame.option) == CLIENT_FINDANONGAME_CANCEL) 
		{
			...
			if (!(rpacket = packet_create(packet_class_bnet))) break;
	...
	case CLIENT_FILEINFOREQ: /* this is exactly the same as the unlogged in packet */
	    if (packet_get_size(packet)<sizeof(t_client_fileinforeq))
	    {
		eventlog(eventlog_level_error,"handle_bnet_packet","[%d] got bad FILEINFOREQ packet (expected %u bytes, got %u)",conn_get_socket(c),sizeof(t_client_fileinforeq),packet_get_size(packet));
		break;
...
*/		    
	...
	case CLIENT_JOIN_GAME:
		...
		if(!strcmp(gamename, "BNet"))
		{
			...
			{
                handle_joingame_whisper(c,gamename);
		...
		else 
		{
			...
			if(conn_get_joingamewhisper_ack(c)==0)
			{
				if(handle_joingame_whisper(c,gamename)==0)
