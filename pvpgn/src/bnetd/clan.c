...
extern int clanlist_save()
{
  ...
  if (clanlist_head)
      ...
      LIST_TRAVERSE(clanlist_head,curr)
	{
      ...
      if (clan->modified) clan_save(clan);
	}
...
extern t_clan *clanlist_find_clan_by_clanid(int cid)
{
  ...
  if (clanlist_head)
      ...
      LIST_TRAVERSE(clanlist_head,curr)
	{
	  ...
	  if (clan->created && (clan->clanid==cid)) return clan;
	}
...
extern t_clan *clanlist_find_clan_by_clanshort(int clanshort)
{
  ...
  if (clanlist_head)
      ...
      LIST_TRAVERSE(clanlist_head,curr)
	{
	  ...
	  if (clan->created && (clan->clanshort==clanshort)) return clan;
	}
...
extern int clanmember_set_online(t_connection * c)
{
  ...
  if((acc=conn_get_account(c)) != NULL && (clan=account_get_clan(acc)) != NULL &&
      (member=clan_find_member(clan, acc)) != NULL)
      {
