...
extern int handle_command(t_connection * c,  char const * text)
{
...
#endif
    /* ADDED BY UNDYING SOULZZ 4/10/02 - Clan Name Setting for Profile */
	if (strstart(text,"/clan")==0)
	{
	t_channel const * channel;
	unsigned int		  i;
	char const *	    clanname;
	...
	if (strstart(text,"/admin") == 0) // [quetzal] 20020815 - rewritten this code
	{
		unsigned int i;
		char msg[255];
		const char *username;
    ...
    // aaron --->
    if (strstart(text,"/remove_accounting_infos")==0)
    {
	if (account_get_auth_admin(conn_get_account(c))!=1) /* default to false */
        {
            message_send_text(c,message_type_error,c,"This command is reserved for admins.");
...
if (strstart(text,"/rank_all_accounts")==0)
    ...
    if (strstart(text,"/set")==0) // MODIFICATION BY DJP ! Should work now ;) ( tested and work ;p )
    {
	t_account * account;
	char *accname;
	char *key;
...
//Below new functions work with the watchlist to notify mutual friends
//When you login and logoff the server 7/27/02 THEUNDYING
