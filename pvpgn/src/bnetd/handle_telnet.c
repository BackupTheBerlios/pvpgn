...
extern int handle_telnet_packet(t_connection * c, t_packet const * const packet)
{
	...
	case conn_state_bot_password:
		...
		{
		    eventlog(eventlog_level_info,"handle_telnet_packet","[%d] bot login for \"%s\" refused (already logged in)",conn_get_socket(c),loggeduser);
		    conn_set_state(c,conn_state_bot_username);
		...
		{
			eventlog(eventlog_level_info,"handle_telnet_packet","[%d] bot login for \"%s\" refused (bad account)",conn_get_socket(c),loggeduser);
			conn_set_state(c,conn_state_bot_username);
